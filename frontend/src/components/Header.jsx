...
const Header = () => {
 ...
 // Close the hamburger when any menu item is clicked
 const closeHamburger = () => {
  setHamburgerOpen(false);
};

  return (
    <header className='fixed w-full z-50 bg-gray-50 dark:bg-gray-800 backdrop-filter backdrop-blur-md px-4 pt-1'>
      ...
      {hamburgerOpen && (
        <div className='absolute top-12 left-0 w-full bg-gray-50 dark:bg-gray-800 lg:hidden p-4 space-y-4' ref={hamburgerRef}>
          <div className='flex flex-col'>

            <Link to={"/contact"} className='block text-black dark:text-white' onClick={closeHamburger}>
              <MdContactPhone className='inline-block mr-2'/> Contact
            </Link>
            <Link to={"/about"} className='block text-black dark:text-white' onClick={closeHamburger}>
            ...
            {user?._id ? (
              ...
              <Link to={"/login"} className='block w-full px-2 py-1 text-center rounded-md text-white bg-red-600 hover:bg-red-700' onClick={closeHamburger}>
                Login
              </Link>
            )}
          </div>
